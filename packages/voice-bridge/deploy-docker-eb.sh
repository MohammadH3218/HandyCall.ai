...
IMAGE_NAME="${VOICE_ECR_REPOSITORY:-handycall-voice-bridge}"
CONTAINER_PORT="${VOICE_CONTAINER_PORT:-8080}"
HOST_PORT="${VOICE_HOST_PORT:-80}"
...
VERSION_TAG="voice-$(date +%Y%m%d-%H%M%S)"
...
docker build --platform linux/amd64 -f packages/voice-bridge/Dockerfile -t "${IMAGE_NAME}:${VERSION_TAG}" .
...
cp -R packages/voice-bridge/.platform "${WORK_DIR}/.platform" 2>/dev/null || true
(cd "${WORK_DIR}" && zip -qr "${VERSION_TAG}.zip" Dockerrun.aws.json .platform)
