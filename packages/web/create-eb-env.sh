...
DOMAIN_NAME="${WEB_DOMAIN_NAME:-handycall.org}"
HOSTED_ZONE_ID="${HOSTED_ZONE_ID:-Z002814819T09BLDX47MG}"
PLATFORM_ARN="${WEB_EB_PLATFORM_ARN:-arn:aws:elasticbeanstalk:us-east-1::platform/Docker running on 64bit Amazon Linux 2023/4.9.0}"
...
if [[ -z "${CERT_ARN}" || "${CERT_ARN}" == "None" ]]; then
  CERT_ARN="$(aws acm request-certificate --domain-name "${DOMAIN_NAME}" --subject-alternative-names "www.${DOMAIN_NAME}" --validation-method DNS --region "$AWS_REGION" --query CertificateArn --output text)"
  sleep 2
...
aws elasticbeanstalk create-environment \
  ...
  --option-settings \
    ...
    "Namespace=aws:elasticbeanstalk:application:environment,OptionName=NEXTAUTH_URL,Value=https://${DOMAIN_NAME}" \
