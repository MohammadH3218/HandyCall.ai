  ...
  return (
    <div className="space-y-6">
        ...
        actions={
          <div className="flex gap-2">
            <button
              ...
              className="inline-flex items-center gap-1.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 px-3 py-2 text-sm font-medium text-slate-700 dark:text-slate-300 shadow-sm hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors"
            >
      ...
      <div className="rounded-2xl border border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm">
        <div className="flex flex-col gap-4 border-b border-slate-100 dark:border-slate-800 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <p className="text-xs font-bold uppercase tracking-widest text-slate-400 dark:text-slate-500">Current Period</p>
            <p className="mt-1 text-sm font-semibold text-slate-900 dark:text-slate-100">
              {usage ? `${formatDate(usage.period_start)} – ${formatDate(usage.period_end)}` : 'No usage recorded for this period.'}
            </p>
          </div>
          {planDetails ? (
            <div className="rounded-xl border border-slate-100 dark:border-slate-800 bg-slate-50/70 dark:bg-slate-800/50 px-4 py-3 text-right">
              <p className="text-xs text-slate-500 dark:text-slate-500">Active plan</p>
              <p className="text-base font-bold text-slate-900 dark:text-slate-100">{planDetails.name}</p>
              <p className="text-xs text-slate-500 dark:text-slate-500">
                {priceDisplay?.original && <span className="mr-1 line-through">{priceDisplay.original}</span>}
                <span className="font-semibold text-slate-700 dark:text-slate-300">{priceDisplay?.current}</span>
        ...
        {/* Meters */}
        <div className="grid gap-0 divide-y divide-slate-50 dark:divide-slate-800 sm:grid-cols-3 sm:divide-x sm:divide-y-0">
          <UsageMeter
            label="Call minutes"
      ...
      <div className="rounded-2xl border border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm">
        <div className="border-b border-slate-100 dark:border-slate-800 px-5 py-4">
          <h2 className="text-sm font-semibold text-slate-900 dark:text-slate-100">Keep your account on track</h2>
          <p className="mt-0.5 text-xs text-slate-500 dark:text-slate-500">Manage your subscription and payment details to avoid interruptions.</p>
        </div>
        <div className="grid gap-px bg-slate-50 dark:bg-slate-800/50 sm:grid-cols-3">
          {[
            {
            ...
            <div key={item.title} className="flex flex-col gap-2 bg-white dark:bg-slate-900 p-5">
              ...
              <p className="text-xs text-slate-500 dark:text-slate-500 flex-1">{item.desc}</p>
              <button
                onClick={item.action}
    ...
    <div className="px-5 py-5">
      <div className="flex items-start justify-between gap-3 mb-4">
        ...
        <div className="text-right">
          <p className="text-xl font-bold text-slate-900 dark:text-slate-100">{usageDisplay}</p>
          {overage > 0 ? (
            <p className="text-xs text-red-600">{overageDisplay}{overageSuffix}</p>
          ) : percent > 0 && limit !== -1 && limit !== undefined ? (
            <p className="text-xs text-slate-400 dark:text-slate-500">{percent}% used</p>
          ) : null}
        </div>
      </div>
