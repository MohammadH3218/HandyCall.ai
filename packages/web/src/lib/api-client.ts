...
class ApiClient {
  ...
  // ─── Marketplace Pro Onboarding ───────────────────────────────────────────

  /** Advances from onboarding step 1 → 2. Uses FormData (required by file interceptor). */
  async proOnboardIdentity(data?: { cr_number?: string; vat_number?: string }): Promise<any> {
    const form = new FormData();
    if (data?.cr_number) form.append('cr_number', data.cr_number);
    if (data?.vat_number) form.append('vat_number', data.vat_number);
    // Must use raw fetch — the request() helper forces application/json which breaks multipart
    const res = await fetch(`${this.baseUrl}/pros/onboarding/identity`, {
      method: 'POST',
      body: form,
      credentials: 'include',
    });
    if (!res.ok) {
      ...
      throw new Error(data?.message || `Identity step failed: ${res.status}`);
    }
  ...
  async proOnboardProfile(data: {
    ...
    const res = await this.request<any>('/pros/onboarding/profile', {
      method: 'POST',
      body: JSON.stringify(data),
    });
    return (res as any)?.data ?? res;
  ...
  async proOnboardServices(data: {
    ...
    const res = await this.request<any>('/pros/onboarding/services', {
      method: 'POST',
      body: JSON.stringify(data),
    });
    return (res as any)?.data ?? res;
  ...
  async proOnboardPayout(data: {
    ...
    const res = await this.request<any>('/pros/onboarding/payout', {
      method: 'POST',
      body: JSON.stringify(data),
    });
    return (res as any)?.data ?? res;
